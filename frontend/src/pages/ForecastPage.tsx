      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/80 border border-slate-800 rounded-2xl p-6">
        <div>
          <h2 className="text-xl font-bold text-slate-100 flex items-center gap-2">
            ...
            <span>Dedicated 72-Hour Air Quality Forecast Engine</span>
          </h2>
          <p className="text-xs text-slate-400">Multi-horizon predictions generated via multi-model XGBoost regressors.</p>
        </div>
          ...
          {(['pm25', 'pm10', 'no2', 'o3', 'aqi'] as const).map(p => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-lg font-bold uppercase transition ${
                selectedPollutant === p ? 'bg-cyan-500 text-white shadow-lg' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              {p}
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 space-y-2">
          <div className="flex items-center justify-between text-xs text-slate-400 font-semibold">
            ...
            <span className="px-2 py-0.5 rounded text-white font-bold" style={{ backgroundColor: h24?.color }}>{h24?.category}</span>
          </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 space-y-2">
          <div className="flex items-center justify-between text-xs text-slate-400 font-semibold">
            ...
            <span className="px-2 py-0.5 rounded text-white font-bold" style={{ backgroundColor: h48?.color }}>{h48?.category}</span>
          </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 space-y-2">
          <div className="flex items-center justify-between text-xs text-slate-400 font-semibold">
            ...
            <span className="px-2 py-0.5 rounded text-white font-bold" style={{ backgroundColor: h72?.color }}>{h72?.category}</span>
          </div>
            ...
            <tbody>
              {timeline.map((row, idx) => (
                <tr key={idx} className="border-b border-slate-800/60 hover:bg-slate-800/40 transition">
                  ...
                  <td className="p-3">
                    <span className="px-2 py-0.5 rounded text-white text-[10px] font-bold" style={{ backgroundColor: row.color }}>
                      {row.category}
                    </span>
                  </td>
